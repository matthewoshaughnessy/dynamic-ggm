...
%   r     : smoothing window size (default: pick best w/ aic)
...
function [fk,df,r] = calculateSpectralDensity(X,r,print)
  ...
  if pickr
    ...
    if print
      fprintf('Specified r = %d manually.\n', r);
    end
  end
  ...
  elseif print
    fprintf('Calculating smoothed periodogram with manually specified value of r...\n');
  end
  fhat = zeros(m,m,nf);
  for k = 1:T
    jj = (-T:T).';
