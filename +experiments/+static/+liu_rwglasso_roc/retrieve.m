% --- retrieve files ---
mkdir(fullfile('+experiments/+static/+liu_rwglasso_roc','results'));
system(['scp -r moshaughnessy6@login-d.pace.gatech.edu:' ...
  '~/data/liu_rwglasso_roc/results* +experiments/+static/+liu_rwglasso_roc/results/'], '-echo');


% --- concatenate results ---
resultfiles = dir('+experiments/+static/+liu_rwglasso_roc/results/results*.mat');
...
load(['+experiments/+static/+liu_rwglasso_roc/results/' resultfiles{1}]);
results = results1;
for i = 2:length(resultfiles)
  fprintf('Concatenating results: %d/%d.', i, length(resultfiles));
  load(['+experiments/+static/+liu_rwglasso_roc/results/' resultfiles{i}]); fprintf('.');
...
save(fullfile('+experiments/+static/+liu_rwglasso_roc','results.mat'),'results','-v7.3');
fprintf('done!\n');


% --- clean up ---
fprintf('Cleaning up...');
system(sprintf('rm -r %s', fullfile('+experiments/+static/+liu_rwglasso_roc','results')));
clear;
fprintf('done!\n');
